...
extension Refresh.Modifier: ViewModifier {
    ...
    func update(proxy: GeometryProxy, value: Refresh.HeaderAnchorKey.Value) {
		...
		if let position = item.position {
			let po = proxy[position]
			print("bounds.minY:\(bounds.minY), bounds.maxY:\(bounds.maxY), bounds.height:\(bounds.height)")
		}
        ...
        headerUpdate = update
		if headerUpdate.refresh {
			headerPadding = 0.0
			print("headerPadding:0.0")
		} else {
			print("rowHeight:\(rowHeight),bounds.height:\(bounds.height)")
			headerPadding = -max(rowHeight, bounds.height)
		}
