...
struct AlscJSONKeyedDecodingContainer<K : CodingKey>: KeyedDecodingContainerProtocol {
	...
	// json 中匹配自定义 key，从左往右，匹配到一个既停止，不管 value 是否匹配
	...
	public func decode<T : Decodable>(_ type: T.Type, forKey key: Key) throws -> T {
		...
		guard let entry = _getEntryBy(key) else {
...
extension AlscJSONKeyedDecodingContainer {
	...
	func decodeIfPresent(_ type: Bool.Type, forKey key: K) throws -> Bool? {
		...
		case .custom(let transformer):
			decoder.storage.push(container: entry)
			defer { decoder.storage.popContainer() }
			return try transformer.transformIfPresent(decoder)
		}
	}
...
private extension String {
	func toJSONObject() -> Any? {
		guard hasPrefix("{") || hasPrefix("[") else { return nil } // 过滤掉非 JSON 格式字符串
