...
public final class ABIEncoder {
    ...
    /// Encodes an address
	public func encode(_ address: web3swift.EthereumAddress) throws {
        let padding = ((address.addressData.count + 31) / 32) * 32 - address.addressData.count
        data.append(Data(repeating: 0, count: padding))
        data.append(address.addressData)
    }
